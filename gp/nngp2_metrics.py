...
with open("results/nngp2_depth"+str(depth)+".json", "r") as fp:
	stride = json.load(fp)["stride"]

print("Loading predictive mean and variance")
with h5py.File("results/nngp2_pred_depth"+str(depth)+".h5", "r") as f:
...
data.to_csv("results/nngp2_metrics_depth"+str(depth)+".csv", index=False)
