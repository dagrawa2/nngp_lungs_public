...
model = models.NNGP2(Q_train_train, Q_train_test, Q_test_test, Y_train, batch_size=batch_size)
del Q_train_train; gc.collect()
del Q_train_test; gc.collect()
del Q_test_test; gc.collect()
del Y_train; gc.collect()
with open("results/nngp2_ho_depth"+str(depth)+".json", "r") as fp:
	hist = json.load(fp)["hist"]
model.v_n = hist["v_n"][-1]
model.v_b = np.array(hist["v_b"][-1])
model.v_w = np.array(hist["v_w"][-1])
model.depth = depth
...
with h5py.File("results/nngp2_pred_depth"+str(depth)+".h5", "w") as f:
...
with open("results/nngp2_depth"+str(depth)+".json", "w") as fp:
