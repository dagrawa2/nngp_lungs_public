...
if depth==0:
	with h5py.File("../gp/results/nngp_pred_depth"+str(depth)+".h5", "r") as f:
		Y_mean = np.array(f["/mean"][::stride])
		Y_std = np.expand_dims(np.sqrt(np.array(f["/var"][::stride])), 1)
else:
	with h5py.File("../gp/results/nngp2_pred_depth"+str(depth)+".h5", "r") as f:
		Y_mean = np.array(f["/mean"][::stride])
		Y_std = np.expand_dims(np.sqrt(np.array(f["/var"][::stride])), 1)
...
aucs.to_csv("results/nngp2_aucs_depth"+str(depth)+".csv", index=False)
...
with open("results/nngp2_info_depth"+str(depth)+".json", "w") as fp:
