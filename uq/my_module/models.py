...
#			fvar = np.tile(fvar.reshape((-1, 1)), (1, self.Y.shape[1]))
	...
	def optimize(self, max_iters=100, print_cb=False):
		...
		history = {"v_n":[self.v_n], "c":[self.c], "l":[self.l], "nle_train":[self.NLE_value/self.A.shape[0]]}
		def f(x):
			x = np.logaddexp(0*x, x)
...
#			fvar = np.tile(fvar.reshape((-1, 1)), (1, self.Y.shape[1]))
	...
	def NLE_gradient(self):
		...
		DNLE = -1/2*W.dot(W.T) + M/2*K_inv
		D_v_n = np.trace(DNLE) *(1.0-np.exp(-self.v_n))
		D_v_b = np.sum(DNLE*D_v_b) *(1.0-np.exp(-self.v_b))
		D_v_w = np.sum(DNLE*D_v_w) *(1.0-np.exp(-self.v_w))
		self.NLE_value = np.sum(NLE)
	...
	def optimize(self, max_iters=100, print_cb=False):
		...
		x0 = np.log(np.exp(x0)-1.0)
		history = {"v_n":[self.v_n], "v_b":[self.v_b], "v_w":[self.v_w], "nle_train":[self.NLE_value/self.A.shape[0]]}
		def f(x):
			x = np.logaddexp(0*x, x)
#			print("f:", np.round(x, 4))
			self.v_n = x[0]
...
#			fvar = np.tile(fvar.reshape((-1, 1)), (1, self.Y.shape[1]))
	...
	def NLE_gradient(self):
		...
		NLE = 1/2*np.sum(self.Y*W) + M/2*list(np.linalg.slogdet(K))[1] + M*N/2*np.log(2*np.pi)
		DNLE = -1/2*W.dot(W.T) + M/2*K_inv
		D_v_n = np.array([np.trace(DNLE)]) *(1.0-np.exp(-self.v_n))
		D_v_b = np.array([np.sum(DNLE*dvb) for dvb in D_v_b]) *(1.0-np.exp(-self.v_b))
		D_v_w = np.array([np.sum(DNLE*dvw) for dvw in D_v_w]) *(1.0-np.exp(-self.v_w))
		self.NLE_value = np.sum(NLE)
	...
	def optimize(self, max_iters=100, print_cb=False):
		...
		x0 = np.log(np.exp(x0)-1.0)
		history = {"v_n":[self.v_n], "v_b":[list(self.v_b)], "v_w":[list(self.v_w)], "nle_train":[self.NLE_value/self.A.shape[0]]}
		def f(x):
			x = np.logaddexp(0*x, x)
#			print("f:", np.round(x, 4))
			self.v_n = x[0]
