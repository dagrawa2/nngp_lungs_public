...
with h5py.File('/raid/scratch/devanshu/innerprods_1024.h5', 'r') as f:
	...
	K_test_test = np.array(f["/test_test"][::stride,::stride])
...
print("Computing cosine similarity kernel . . . ")
K_train_norms = np.sqrt(np.diagonal(K_train_train))
del K_train_train; gc.collect()
K_test_norms = np.sqrt(np.diagonal(K_test_test))
del K_test_test; gc.collect()
K_train_test = K_train_test/np.outer(K_train_norms, K_test_norms)
...
aucs.to_csv("results/cos_aucs.csv", index=False)
accs.to_csv("results/cos_accs.csv", index=False)
...
with open("results/cos_info.json", "w") as fp:
